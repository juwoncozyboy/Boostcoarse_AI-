...
def process_submit():
    ...
    # 제출 파일 설정
    file_name = "(중급) 29차시_프로젝트 9 제출코드(RNN).ipynb"
    submit_file = project_path / file_name  # 직접 파일 경로 설정

    # 평가 기준 파일 설정 (분할 없이 직접 지정)
    submit_rubric_file = Path("/Users/juwonkim/Desktop/네이버 부스트코스 AI 엔지니어/week66/03_rnn_pt/check_util/rnn_submission.tsv")
